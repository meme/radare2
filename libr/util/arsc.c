...
enum {
	...
	TYPE_RES_TABLE = 0x0002,
	TYPE_RES_TABLE_PACKAGE = 0x0200,
};
...
// String pool referenced throughout the Binary Resource, there must only be ONE
R_PACKED (
typedef struct {
	ut32 string_count;
	ut32 style_count;
...
R_API char *r_arsc_decode(const ut8 *data, const ut64 data_size) {
	...
	ut32 package_count = 0;
	if (r_buf_fread_at (buffer, offset, (ut8 *)&package_count, "i", 1) != sizeof(package_count)) {
		goto bad;
	}
	...
	offset += sizeof(package_count);

	sb = r_strbuf_new ("");

// <resources>
//     <public type="anim" name="abc_fade_in" id="2130771968" />
...
bad:
	eprintf ("Invalid Android Binary Resource\n");
error:
	if (buffer)
		r_buf_free (buffer);
	free (pool);
